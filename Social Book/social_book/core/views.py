...
def index(request):
    ...
    # followed user profile picture
    posts_with_profile = []

    for post in feed_list:
        profile = Profile.objects.get(user__username=post.user)
        posts_with_profile.append({
            'post': post,
            'profile': profile
        })

    # usetr suggestion starts
...
def signin(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.info(request, 'You are not registered, please register first')
...
def delete_comment(request, comment_id):
    ...
    if request.user.username == comment.user or request.user.username == post.user:
        comment.delete()
